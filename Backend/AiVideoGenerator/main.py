...
# Convert Gemini scene data into Python JSON
clean_scene_data = scene_data.strip()

if clean_scene_data.startswith("```"):
    clean_scene_data = clean_scene_data.replace("```json", "", 1)
    clean_scene_data = clean_scene_data.replace("```", "", 1).strip()

scenes = json.loads(clean_scene_data)

print("Number of scenes:", len(scenes["scenes"]))
...
# -----------------------------
# 8. Generate voice for each scene
# -----------------------------
...
for scene in scenes["scenes"]:
    ...
    audio_file = f"audio/scene_{scene_number}.mp3"

    print(f"Generating voice for Scene {scene_number}...")

    asyncio.run(
        generate_voice(narration, audio_file)
    )

    print(f"Voice created: {audio_file}")

print("\nAll scene voices generated successfully!")
